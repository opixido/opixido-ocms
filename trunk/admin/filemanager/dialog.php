...
//php sorting
            ...
            foreach ($files as $k => $file) {
                ...
                elseif (is_dir($current_path . $rfm_subfolder . $subdir . $file)) {
                    ...
                    if ($subdir != '' || ( $file == $sharedFolder || $file == $_SESSION['gs_admin_id'] )) {
                        $sorted[$k] = array('file' => $file, 'date' => $date, 'size' => $size, 'extension' => $file_ext);
                        if ($file == $_SESSION['gs_admin_id']) {
                            $sorted[$k]['realname'] = 'Mes fichiers';
                        }
                    }
                } else {
        ...
        <?php if (isset($folder_message)) { ?>
                        ...
                        <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                            ...
                            foreach ($files as $file_array) {
                                ...
                                ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $get_params . rawurlencode($src) . "&" . uniqid() ?>"><?php echo empty($file_array['realname']) ? $file : $file_array['realname']; ?></a></h4>
                                            </div>
