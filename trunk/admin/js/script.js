...
function smallPopup(href) {
	...
	window.open(href,'test','width=200,height=200,scrollbars=yes,resizable=yes');
...
function genformPreviewFk(curtable,nom,champs,iframenom) {
    ...
    obj = gid('genform_'+nom) ? gid('genform_'+nom) : gid(nom);
...
function showInfos(info){
	/*gid('info_picto').innerHTML = info;
	gid('info_picto').style.left = mouseX+15;
	gid('info_picto').style.top = mouseY+15;
...
function insertDate(obj) {
	var myDate=new Date();
	obj.value += myDate.getDate()+"/"+myDate.getMonth()+"/"+myDate.getFullYear();
}


function insertTime(obj) {
	var myDate=new Date();
	obj.value += myDate.getHours()+"/"+myDate.getMinutes()+"/"+myDate.getSeconds();
}
	...
	if(direct > 0) {
		...
		*/
		for (i = 1; i < tbox.options.length; i++) {
			if(tbox.options[i].selected && tbox.options[i+direct] && (i+direct) > -1 ) {
