...
tinymce.PluginManager.add('mediawithclassvideo', function(editor, url) {
    console.log(editor);
    editor.on('keydown', function(e) {
        $(editor.dom.doc).find('.video').each(function() {
            var node = this;
            if ($(node).hasClass('video')) {
                ...
                if ($(node).find('iframe').length === 0
                        ...
                        && $(node).find('embed').length === 0
                        && $(node).find('.mce-object').length === 0
                        && $(node).find('.mce-object-iframe').length === 0
                        )
                {
                    $(node).removeClass('video');
                }
            }
        });
        /*if (e.keyCode === 13) {
         tinymce.each(editor.dom.getParents(editor.selection.getNode()), function(node) {
         if ($(node).hasClass('video')) {
         console.log($(node).html());
         if ($(node).find('iframe').length === 0
         && $(node).find('video').length === 0
    ...
    editor.addButton('mediawithclassvideo', {
        icon: 'media',
    ...
    editor.addMenuItem('mediawithclassvideo', {
        icon: 'media',
        text: 'Insert video',
        onclick: showDialog,
        context: 'insert',
