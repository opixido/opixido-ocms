...
function getPagination ($nbPages) {
	 ...
	 */
	$tpl = new genTemplate(true);
	$tpl->loadTemplate('pagination','template');
	
	/**
	 * Paramètres existants pour ne pas perdre les autres données
	 */
	$params = $GLOBALS['site']->g_url->paramsUrl;
	...
	if($page == 1) {
		$tpl->set('prec_url','');
		$tpl->set('prec_class','off');
	} else {
		$tpl->set('prec_url',getUrlWithParams(array_merge($params,array('page'=>($page-1)))));
		$tpl->set('prec_class','on');
	}
	 ...
	 */
	if(($page == $nbPages )) {
		$tpl->set('suiv_class','off');
		$tpl->set('suiv_url','');
	} else {
		$tpl->set('suiv_class','on');
		$tpl->set('suiv_url',getUrlWithParams(array_merge($params,array('page'=>($page+1)))));
	}
	...
	for($p=1;$p<=$nbPages;$p++) {
		...
		if($nbPages > 10) {
			if(( $p == 1 ||  $p == $nbPages ||  $p == $page || ($p % 10 == 0) || abs($page - $p) < 3)) {
		...
		$num->set('num',$p);
		$num->set('url',getUrlWithParams(array_merge($params,array('page'=>($p)))));
