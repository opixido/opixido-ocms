...
/*	function gen() {
	...
	function addContactList(){
		 ...
		 */
		if(count($tabC) > 2) {
			$this->form->add('select',$tabC,t('c_qui'),'c_qui',false,true,array($_REQUEST['c_qui']?$_REQUEST['c_qui']:'null'));
		} else {
	...
	function ocms_getParams() {
		...
		$params['contact_show_email'] = array('select',array('yes','no'));
		$params['contact_show_comment'] = array('select',array('yes','no'));
		$params['contact_email_position'] = array('select',array('top','bottom'));
