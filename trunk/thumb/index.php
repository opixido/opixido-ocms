...
if(!$_GET['src']) {
	...
	if(!$_SESSION['cimg_'.md5($_SERVER['REQUEST_URI'])] || !file_exists($_SESSION['cimg_'.md5($_SERVER['REQUEST_URI'])])) {
		...
		if($_REQUEST['table'] && $_REQUEST['champ'] && $_REQUEST['id']) {
			$gf = new GenFile($_REQUEST['table'],$_REQUEST['champ'],$_REQUEST['id']);
			$_GET['src'] = $_REQUEST['src'] = $gf->getSystemPath();
			...
			if(strpos($url,'./'))
				...
				$_GET['src'] = $_REQUEST['src'] = '..'.$url;
				
				
		}else {
			$_SESSION['cimg_'.md5($_SERVER['REQUEST_URI'])]='';
			die();
		}
		
		$_SESSION['cimg_'.md5($_SERVER['REQUEST_URI'])] = $_REQUEST['src'];
...
//ob_start();
if (!include(dirname(__FILE__).'/phpthumb.class.php')) {
	//ob_end_flush();
	die('failed to include("'.realpath(dirname(__FILE__).'/phpthumb.class.php').'")');
}
//ob_end_clean();
...
if (file_exists(dirname(__FILE__).'/phpThumb.config.php')) {
	ob_start();
	if (include(dirname(__FILE__).'/phpThumb.config.php')) {
		// great
	} else {
		ob_end_flush();
		$phpThumb->ErrorImage('failed to include('.dirname(__FILE__).'/phpThumb.config.php) - realpath="'.realpath(dirname(__FILE__).'/phpThumb.config.php').'"');
	}
	ob_end_clean();
...
header('X-Cache-Type:disabled');
	...
	if ((file_exists($phpThumb->cache_filename) && is_writable($phpThumb->cache_filename)) || is_writable(dirname($phpThumb->cache_filename))) {
		header('X-Cache-Type:Caching');
		if(date('H') == 5 && rand(0,20) == 10) {
		...
		header('X-Cache-Type:Cannotcache');
