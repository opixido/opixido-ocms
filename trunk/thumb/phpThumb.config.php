...
$PHPTHUMB_CONFIG['cache_maxsize'] = 1000 * 1024 * 1024 *1024; // delete least-recently-accessed cached thumbnails when more than [10MB] of cached files are present (value is maximum bytesize of all cached files)

//$PHPTHUMB_CONFIG['cache_maxfiles'] = null;          // never delete cached thumbnails based on number of cached files
$PHPTHUMB_CONFIG['cache_maxfiles'] = 100000;             // delete least-recently-accessed cached thumbnails when more than [200] cached files are present (value is maximum number of cached files to keep)
...
$PHPTHUMB_CONFIG['error_message_image_default'] = '/img/pictos/mimetypes/image-x-generic.png';       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
...
$PHPTHUMB_CONFIG['disable_onlycreateable_passthru'] = false;   // if true, any image that can be parsed by GetImageSize() can be passed through; if false, only images that can be converted to GD by ImageCreateFrom(JPEG|GIF|PNG) functions are allowed
...
$PHPTHUMB_DEFAULTS_GETSTRINGOVERRIDE = false;  // if true, any parameters in the URL will override the defaults set here; if false, any parameters set here cannot be overridden in the URL
