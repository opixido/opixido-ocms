...
class indexSearch {
	...
	function __construct($obj='') {
		...
		if($obj) {
			$this->obj = $obj;
			$this->tab = $_Gconfig['iSearches'][$obj];
		...
		foreach($this->tab['champs'] as $field) {
			...
			if(ake($this->fields,$field)) {
				...
				if(arrayInWord( $uploadFields, $field ) || arrayInWord($_Gconfig['urlFields'],$field)) {
					...
					$coef = 1;
					if($_Gconfig['searchRatio'][$this->obj][$field]) {
	...
	function insertDict($word) {
		...
		debug('XXXXXXXXX');
		diebug('CCCCCCCC');
		mail('celio@opixido.com','TEST '.time(),'TEST','From: celio@opixido.com');
*/
	...
	function cleanWord($word) {
		...
		*/
		...
		if(!$this->cachestem[$ow]) {



			if($this->stemSpecialWords[$ow]) {
	...
	function search ($q,$select="",$from="",$where="",$order="") {
		...
		if(count($words)> 0 && $words[0] != "") {
			...
			$sql = 'SELECT O.* ,
						...
						FROM '.$this->obj.' AS O, os_obj as IO , os_rel AS IR, os_word AS IW '.$from.'
