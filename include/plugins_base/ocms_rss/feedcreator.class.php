...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= $this->_createGeneratorComment();
		$feed.= $this->_createStylesheetReferences();
		$feed.= "<rss xmlns:media='http://search.yahoo.com/mrss/' version=\"".$this->RSSVersion."\">\n"; 
		$feed.= "    <channel>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			...
			if ($this->RSSVersion == "2.0" && $this->items[$i]->image!="") {
				$feed.= "            <media:thumbnail url='".htmlspecialchars($this->items[$i]->image)."' />\n";
			}
