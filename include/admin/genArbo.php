...
class genArbo {
  ...
  private $rubs;
	...
	public function __construct($idRub) {
      ...
      $this->idCurRub = $idRub;
      $this->rubs = genAdmin::getRubs();
      $this->rubs = $this->rubs[$this->idCurRub];
  ...
  function createRubs($rub, $idParentRedac){
      ...
      if(isset($rub[value])){
			       ...
			       */
			      /*
			      $sql = "INSERT INTO s_rubrique ( fk_rubrique_id,  rubrique_etat,  fk_creator_id, rubrique_ordre) VALUES (".$idParentOnline.",'en_ligne',".$gs_obj->adminid.",".$ordre.");";
			      doSql($sql);
			      */
			      
			      // **** version en rédaction...
			      /*
			      $sql = 'SELECT MAX(rubrique_id) FROM s_rubrique WHERE rubrique_etat=\'en_ligne\';';
			      $res = getSingle($sql);
			      $idVersion = $res['MAX(rubrique_id)'];
			      */      
			      $fields = '';
			      $values = '';
