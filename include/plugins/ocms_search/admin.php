...
function viderRecherche () {
		...
		echo 'Objets : '.DoSql($sql).'<br/>';
		$sql = 'TRUNCATE TABLE os_rel';
		echo 'Relations : '.DoSql($sql).'<br/>';
		$sql = 'TRUNCATE TABLE os_word';
		echo 'Mots : '.DoSql($sql).'<br/>';
...
function indexTable($table) {
	global $_Gconfig;
			...
			$sql = 'SELECT '.$pk.' FROM '.$table.' WHERE 1 '.sqlOnlyOnline($table);
			$res = GetAll($sql);
			$nbt++;
			p('<script type="text/javascript">
			pbar = gid("pbar");
			//ris1.value = "Indexing : '.$table.' ('.$nbt.'/'.count($tables).')"
			</script>');
			...
			foreach($res as $row) {
					set_time_limit(30);
					if($table == 's_rubrique' && !isRubriqueRealAndOnline($row[$pk]))  {
						$nb--;
						$pc = ceil(($nbm/$nb)*100);
...
function indexForSearch($id,$row=array(),$obj,$table) {
	...
	if(in_array($table,$tables)) {
		...
		if($table == 's_rubrique' && !isRubriqueRealAndOnline($id)) {
			...
			if($row['fk_rubrique_version_id'] && isRubriqueRealAndOnline($row['fk_rubrique_version_id'])) {
				return indexForSearch($row['fk_rubrique_version_id'],getRowFromId('s_rubrique',$row['fk_rubrique_version_id']),$obj,$table);
			}
			else {
				return;
			}
