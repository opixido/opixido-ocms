...
foreach($tabForms as $table=>$vals) {
	if(!is_array($tabForms[$table]['pages'])) {
...
class ocms_loginAdmin {
	...
	function __construct() {
		...
		} else if($_REQUEST['xhr'] == 'laAddUser') {
		...
		} else if($_REQUEST['xhr'] == 'laDelUser') {
	...
	function ocms_getParams() {
		...
		$params['canCreate'] = array('select',array('yes','no'));
