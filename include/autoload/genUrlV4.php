...
class genUrlV4 {
    ...
    function parseUrl() {
        ...
        if ($_Gconfig['onlyOneLgForever']) {
			...
			$temp_host = $_SERVER['HTTP_HOST'];
			$temp_host = explode('.', $temp_host);
			
            $templg = array_shift($temp_host);
...
//@todo remettre cache
        ...
        if (isset($GLOBALS['tabUrl'][$rubId]) && $GLOBALS['tabUrl'][$rubId]['type'] == 'link') {
			...
			if(
				...
				$real_host = preg_replace('/'.$this->lg.'\./','',$GLOBALS['tabUrl'][$rub]['webroot'],1);
			}
			
			if($reallg != LG_DEF){
				$url = path_concat('http://'.$reallg.'.'.$real_host, $url);			
			}else{
				$url = path_concat('http://'.$real_host, $url);
			}*/
        ...
        if ($this->curLinkRoot && false) {
            ...
            if (
				...
				if($reallg != LG_DEF){
					$url = path_concat($_Gconfig['protocole'] . '://'.$reallg.'.'.$real_host, $url);			
				}else{
					$url = path_concat($_Gconfig['protocole'] . '://'.$real_host, $url);
				}
    ...
    function buildUrl($rubId, $lg) {
        ...
        if (isset($GLOBALS['tabUrl'][$rubId]) && $GLOBALS['tabUrl'][$rubId]['type'] == 'folder' && $rubId != $this->root_id && $GLOBALS['tabUrl'][$rubId]['type'] != RTYPE_SITEROOT && $this->downArbo) {
            ...
            if (rubHasOption($GLOBALS['tabUrl'][$rubId]['option'], 'dynSubRubs')) {
                ...
                if ($subs) {
                    return array(path_concat('/', $url), array($subs[0]['PARAM'] => $subs[0]['VALUE']));
                } else {
                    return array(path_concat('/', $url));
                }
            } else {
                $subs = $this->recursRub($subId, 1, 1);
            }
