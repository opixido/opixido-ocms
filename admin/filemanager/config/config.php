...
$base_url = '';
$upload_dir = '/fichier/rte/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../fichier/rte/'; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = '../c/'; // relative path from filemanager folder to thumbs folder (with final /)

$sharedFolder = 'General';
ini_set('display_errors', 'on');
if (!file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $_SESSION['gs_admin_id'])) {
    mkdir(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $_SESSION['gs_admin_id']);
    file_put_contents(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $_SESSION['gs_admin_id'] . '/config.php', '<?php if($_SESSION[\'gs_admin_id\'] == "' . ($_SESSION['gs_admin_id']) . '") { $delete_files = true;$create_folders = true;$delete_folders = true;$upload_files = true;$rename_files = true;$rename_folders = true;$duplicate_files = true;} else { $delete_files = false;$create_folders = false;$delete_folders = false;$upload_files = false;$rename_files = false;$rename_folders = false;$duplicate_files = false;}');
}
if (!file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $sharedFolder)) {
    mkdir(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $sharedFolder);
    file_put_contents(dirname(__FILE__) . DIRECTORY_SEPARATOR . '../' . $current_path . $sharedFolder . '/config.php', '<?php if($_SESSION[\'superAdmin\']) { $delete_files = true;$create_folders = true;$delete_folders = true;$upload_files = true;$rename_files = true;$rename_folders = true;$duplicate_files = true;} else { $delete_files = false;$create_folders = false;$delete_folders = false;$upload_files = false;$rename_files = false;$rename_folders = false;$duplicate_files = false;}');
}
