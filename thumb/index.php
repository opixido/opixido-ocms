...
if($_GET['sSrc']) {
	session_start();
	$_GET['src'] = $_SESSION['thumbMD5'][$_GET['sSrc']];
	if(!$_GET['src']) {
		print_r($_SESSION['thumbMD5']);
		die('SESSIOn empty');
	}
	/*echo $_GET['src'] ;
	die();*/
...
// deprecated: 'err', 'file', 'goto',
$allowedGETparameters = array('sSrc','src', 'new', 'w', 'h', 'wp', 'hp', 'wl', 'hl', 'ws', 'hs', 'f', 'q', 'sx', 'sy', 'sw', 'sh', 'zc', 'bc', 'bg', 'bgt', 'fltr', 'xto', 'ra', 'ar', 'aoe', 'far', 'iar', 'maxb', 'down', 'phpThumbDebug', 'hash', 'md5s', 'sfn', 'dpi', 'sia', 'nocache');
...
if ($phpThumb->rawImageData) {
	...
	$phpThumb->ErrorImage('1Usage: '.$_GET['src'].'/'.$phpThumb->src.' / '.$_SERVER['PHP_SELF'].'?src=/path/and/filename.jpg'."\n".'read Usage comments for details');
