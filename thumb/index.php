...
if (false && !empty($_COOKIE['resolution'])) {
    list($taille, $ratio) = explode(',', $_COOKIE['resolution']);
    if ($ratio > 1) {
        /**
         * Si on a un ecran retina on augmente la definition de l'image
         */
        $_GET['w'] = $_REQUEST['w'] = round($_GET['w'] * $ratio);
        $_GET['h'] = $_REQUEST['h'] = round($_GET['h'] * $ratio);
        $_SERVER['REQUEST_URI'] .= '&fltr[]=ratio' . $ratio;
    }

    /**
     * Si on demande une image plus grande que la taille de l'écran ..
     * On la retrecie
     */
    if ($_GET['w'] * $ratio > $taille) {
        $ratio = 1;
        if (!empty($_GET['zc'])) {

            # Si on demande une taille exacte
            # On applique le ratio initial             
            $ratioIMG = $_GET['w'] / $_GET['h'];
            $_GET['w'] = $_REQUEST['w'] = round($taille * $ratio);
            $_GET['h'] = $_REQUEST['h'] = round($_GET['w'] / $ratioIMG);
            $_SERVER['REQUEST_URI'] .= '&fltr[]=size' . $_GET['w'] . 'x' . $_GET['h'];
        } else {
            # Sinon on ne change que la largeur
            $_GET['w'] = $_REQUEST['w'] = $taille * $ratio;
            $_SERVER['REQUEST_URI'] .= '&fltr[]=size' . $_GET['w'] . 'x' . $_GET['h'];
        }
    }
}
